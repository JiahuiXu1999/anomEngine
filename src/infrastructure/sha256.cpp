...
public:
    ...
    std::array<std::uint8_t, 32> finish() {
        totalBits_ += static_cast<std::uint64_t>(bufferSize_) * 8U;
        ...
        for (int shift = 56; shift >= 0; shift -= 8) {
            buffer_[bufferSize_++] = static_cast<std::uint8_t>(totalBits_ >> shift);
        }
        transform();
...
private:
    void transform() {
        ...
        for (std::size_t i = 0; i < schedule.size(); ++i) {
            ...
            const std::uint32_t temporary1 = h + sum1 + choose + kRoundConstants[i] + schedule[i];
...
Result<std::string> sha256File(const std::filesystem::path& path) {
    ...
    if (!stream) return Status::error(ErrorCode::IoError, "Unable to open artifact for SHA-256", path.string());
    Sha256 hash;
    ...
    if (!stream.eof()) return Status::error(ErrorCode::IoError, "Unable to read artifact for SHA-256", path.string());
