...
> [!NOTE]
> 本文是 C ABI 改造前的历史设计草案，用于保留架构演进背景。当前实现已经采用运行时动态加载的算法与后端插件，并在 ABI v2 中改为每种算法对应独立结构体；涉及公共调用方式、静态注册、目标名称和交付布局的内容均已被现有源码、根目录 README、`src/model/README.md` 和 `docs/algorithm-object-api.md` 取代。
