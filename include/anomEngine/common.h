#ifndef ANOM_ENGINE_COMMON_H
#define ANOM_ENGINE_COMMON_H
...
/* Explicit C calling convention, independent of the client's /Gd or /Gz. */
#if defined(_WIN32)
#  define ANOM_CALL __cdecl
#else
#  define ANOM_CALL
#endif
...
ANOM_ENGINE_API uint32_t ANOM_CALL anom_get_abi_version(void);
ANOM_ENGINE_API const char* ANOM_CALL anom_get_version_string(void);

ANOM_ENGINE_API size_t ANOM_CALL anom_algorithm_get_count(void);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_algorithm_get_info(
    size_t index,
    anom_algorithm_info_t* out_info);

ANOM_ENGINE_API anom_status_t ANOM_CALL anom_model_open(
    const char* model_package_utf8,
    anom_model_t** out_model);

ANOM_ENGINE_API anom_status_t ANOM_CALL anom_model_get_info(
    const anom_model_t* model,
    anom_model_info_t* out_info);

ANOM_ENGINE_API anom_status_t ANOM_CALL anom_model_validate(
    const char* model_package_utf8,
    const anom_model_validate_options_t* options,
    anom_model_validation_report_t* out_report);

ANOM_ENGINE_API void ANOM_CALL anom_model_destroy(anom_model_t* model);

ANOM_ENGINE_API anom_status_t ANOM_CALL anom_package_builder_create(
    const anom_package_builder_options_t* options,
    anom_package_builder_t** out_builder);

ANOM_ENGINE_API anom_status_t ANOM_CALL anom_package_builder_add_artifact(
    anom_package_builder_t* builder,
    const char* source_path_utf8,
    const char* package_relative_path_utf8);

ANOM_ENGINE_API anom_status_t ANOM_CALL anom_package_builder_commit(
    anom_package_builder_t* builder,
    const char* output_package_utf8);

ANOM_ENGINE_API anom_status_t ANOM_CALL anom_package_builder_apply_calibration(
    anom_package_builder_t* builder,
    const anom_calibration_result_t* calibration);

ANOM_ENGINE_API void ANOM_CALL anom_package_builder_destroy(
    anom_package_builder_t* builder);

ANOM_ENGINE_API anom_status_t ANOM_CALL anom_calibrator_create(
    const anom_model_t* model,
...
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_calibrator_add_batch(
    anom_calibrator_t* calibrator,
    const anom_image_t* images,
    size_t image_count);

ANOM_ENGINE_API anom_status_t ANOM_CALL anom_calibrator_compute(
    const anom_calibrator_t* calibrator,
    anom_calibration_result_t* out_result);

ANOM_ENGINE_API void ANOM_CALL anom_calibrator_destroy(anom_calibrator_t* calibrator);
 ...
 * New applications should use the objects in the algorithm-specific headers.
 */
...
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_session_create(
    const char* model_package_utf8, const anom_session_options_t* options,
    anom_session_t** out_session);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_session_create_v2(
    const char* model_package_utf8, const anom_session_options_v2_t* options,
    anom_session_t** out_session);
ANOM_ENGINE_API void ANOM_CALL anom_session_destroy(anom_session_t* session);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_session_warmup(anom_session_t* session);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_session_get_model_info(
    const anom_session_t* session, anom_model_info_t* out_info);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_session_get_execution_info(
    const anom_session_t* session, anom_execution_info_t* out_info);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_session_predict(
    anom_session_t* session, const anom_image_t* image,
    anom_prediction_t* out_prediction);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_session_predict_batch(
    anom_session_t* session, const anom_image_t* images, size_t image_count,
    anom_prediction_t* out_predictions);

ANOM_ENGINE_API anom_status_t ANOM_CALL anom_fitter_create(
    const anom_fitter_options_t* options, anom_fitter_t** out_fitter);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_fitter_add_batch(
    anom_fitter_t* fitter, const anom_image_t* images, size_t image_count);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_fitter_get_progress(
    const anom_fitter_t* fitter, anom_fit_progress_t* out_progress);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_fitter_save_checkpoint(
    const anom_fitter_t* fitter, const char* checkpoint_path_utf8);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_fitter_load_checkpoint(
    anom_fitter_t* fitter, const char* checkpoint_path_utf8);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_fitter_cancel(anom_fitter_t* fitter);
ANOM_ENGINE_API anom_status_t ANOM_CALL anom_fitter_finalize(
    anom_fitter_t* fitter, const char* output_package_utf8);
ANOM_ENGINE_API void ANOM_CALL anom_fitter_destroy(anom_fitter_t* fitter);
#endif

ANOM_ENGINE_API void ANOM_CALL anom_prediction_release(anom_prediction_t* prediction);

/* Returns the required byte count including the trailing NUL. */
ANOM_ENGINE_API size_t ANOM_CALL anom_get_last_error(char* buffer, size_t buffer_size);
